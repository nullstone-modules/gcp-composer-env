...
resource "google_storage_bucket_iam_member" "pusher_object_admin" {
  bucket = local.dag_gcs_bucket
  role   = "roles/storage.objectAdmin"
  member = "serviceAccount:${google_service_account.pusher.email}"
}

resource "google_service_account_iam_binding" "pusher_impersonators" {
  service_account_id = google_service_account.pusher.id
